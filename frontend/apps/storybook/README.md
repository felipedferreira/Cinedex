...
The [Storybook](https://storybook.js.org/) for Cinedex's three component tiers — [`@cinedex/atoms`](../../packages/atoms/README.md), [`@cinedex/compounds`](../../packages/compounds/README.md) and [`@cinedex/solution`](../../packages/solution/README.md) — a workbench for building and reviewing them in isolation.
...
flowchart LR
    ...
    CONF --> C2["preview.tsx<br/><i>theme CSS, SolutionProvider, a11y parameters, theme toolbar</i>"]

    SRC --> S1["atoms/*.stories.tsx"]
    SRC --> S2["compounds/*.stories.tsx"]
    SRC --> S3["solution/Screens.stories.tsx"]
```

Stories are grouped by tier, which is also how they appear in the sidebar: **Atoms**, **Compounds**, **Solution**.
...
`preview.tsx` wraps every story in `<SolutionProvider>` with **no `linkComponent`**, so `@cinedex/solution`'s screens fall back to plain anchors. That is why a full sign-in screen renders here with no router and no mock — the point of injecting navigation rather than importing it.

## ➕ Adding a story

1. Create `src/<tier>/<Name>.stories.tsx`.
2. Import the component from `@cinedex/atoms`, `@cinedex/compounds` or `@cinedex/solution` — **never** by a relative path into `packages/`. If the import fails, the component is missing from that package's barrel; add it there.
3. Use CSF3 with `satisfies Meta<typeof X>` and `tags: ['autodocs']` so it appears in the generated docs.
4. Title it `Atoms/…`, `Compounds/…` or `Solution/…` to land in the right sidebar group.
