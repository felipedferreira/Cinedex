...
### Added
- **The SPA runs under the Aspire AppHost (`AddCinedexFrontendUi`)** - `dotnet run --project aspire/Cinedex.AppHost` now starts `frontend/apps/cinadex-ui`'s Vite dev server alongside Postgres, Mailpit and the three .NET hosts, so the inner loop covers the whole stack rather than the backend half of it. Previously the SPA was only part of the Compose path, and using it with the AppHost meant a second terminal running `npm run dev` and remembering that its `/movies-svc` proxy defaults to the bare `dotnet run` address, not the one the AppHost gives the web service.
...
- **Storybook for the component library** - Storybook 10 with `@storybook/react-vite`, `addon-docs` for generated docs and `addon-a11y` for accessibility checks, on port 6006 because the app holds 9000 with `strictPort: true`. `npm run storybook` from `frontend/`. It loads the package's own `vite.config.ts`, so stories compile through the same React Compiler Babel preset the app uses rather than an approximation of it, and `staticDirs` points at the app's `public/` so the `/icons.svg#id` sprite resolves in stories. CI runs `build-storybook`, so a broken story or a bad config fails the build rather than being discovered later.

### Changed
- **`frontend/` is now an npm workspace (`apps/*` + `packages/*`)** - The SPA moved from `frontend/cinadex-ui/` to `frontend/apps/cinadex-ui/` and shares the tree with `frontend/packages/ui/`. The single `package-lock.json` and all shared tooling config — `eslint.config.js`, `.prettierrc.json`, `.prettierignore`, `.gitignore`, `.dockerignore` — are hoisted to `frontend/`, which is where every npm command now runs. Lint and format make one pass over both packages; build, test and coverage fan out with `--workspaces`; `-w cinadex-ui` or `-w @cinedex/ui` scopes any of them to one package.
  ...
  - **The Docker build context widened to `frontend/`,** because that is where the lockfile now is. `compose.yaml` gained an explicit `dockerfile: apps/cinadex-ui/Dockerfile` to go with it, and the Dockerfile copies each workspace manifest before running `npm ci` so the install layer stays cached until a dependency actually changes. The install is scoped with `--workspace cinadex-ui --include-workspace-root`, so building the production image does not pull down the Storybook and test toolchains that only `@cinedex/ui` needs. The `cinadex-ui` service and container names are deliberately unchanged — `backend/docker-compose/Properties/launchSettings.json` and the `.dcproj` both refer to them.
  ...
  - **The Aspire AppHost's SPA resource follows the move.** `AppHostConstants.FrontendAppDirectory` now points at `frontend/apps/cinadex-ui`. It deliberately stays on the package rather than the workspace root: `npm run dev` there resolves to plain `vite`, so the `--port` Aspire appends reaches it, which it would not through the root script's `-w cinadex-ui` delegation.
