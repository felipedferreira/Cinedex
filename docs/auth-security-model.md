...
whose implementation lives in the `Cinedex.Email.Smtp` adapter. The only registered implementation
is `NoOpEmailSender`, so **reset emails are not actually delivered**. Password reset is not usable
