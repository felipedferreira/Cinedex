...
- `nginx.conf` only serves the SPA and its history fallback on internal port 8080. `Caddyfile` owns the Compose `/movies-svc` route to `movies.webservice`; a backend base-path change must be mirrored there and in `vite.config.ts`.
