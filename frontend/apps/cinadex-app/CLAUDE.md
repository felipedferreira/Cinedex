...
- **`main.tsx` imports `@cinedex/components/tokens.css` and `@cinedex/components/base.css` before `./index.css`.** Design tokens and base element styling belong to the library; only app-specific layout lives in `src/index.css`.
