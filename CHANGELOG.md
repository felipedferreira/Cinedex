...
- **Scheduled background jobs (`Cinedex.SchedulerWorker`)** - A new .NET Generic Host presentation project for work that runs on a timer rather than on a request, added to Compose as `movies.schedulerworker`. It gates startup on `movies.databasemigrator` completing so a job can never run against an unmigrated schema, and serves no HTTP traffic.
  - **OpenTelemetry setup is now shared** — logging and tracing configuration moved into a new packable `Cinedex.Observability.OpenTelemetry` library exposing one `AddObservability` extension on `IHostApplicationBuilder`, which both `HostApplicationBuilder` and `WebApplicationBuilder` implement. The web service, the migrator and the scheduler worker now all export to Seq through the same code path, each passing only its own instrumentation; the three near-identical copies are gone. Exporters are still omitted when no OTLP endpoint is configured, so a local `dotnet run` does not reach for a Seq that isn't there.
...
### Changed
- **Refresh-token persistence split into a repository and queries (`Cinedex.Auth.Identity.Persistence`)** - Every refresh-token database operation was written inline against `AuthDbContext`, spread across `JwtTokenService`, `IdentityService` and `RefreshTokenCleanupWorker`, so there was no single place to see what the system does to `auth."refreshTokens"` and EF query syntax sat in the middle of JWT-minting logic. Writes now go through `IRefreshTokenRepository` in `Persistence.Repository` and reads through `IRefreshTokenQueries` in `Persistence.Query`, each backed by its own `DbContext` — the new `AuthReadOnlyDbContext` maps the same model as `AuthDbContext` from a shared `AuthModel.Configure`, so the two cannot drift. Queries return a detached `RefreshTokenReadModel` from `Persistence.ReadModels`, never the persistence entity or an `IQueryable`; the repository returns `RefreshToken` entities from `Persistence.Entities` for authoritative reads on the write connection. Behavior is unchanged; the split is structural.
  - **The query side is thin on purpose.** Only one refresh-token read is correct outside a transaction — the preflight lookup that lets `RefreshAsync` reject unknown and expired tokens without opening one. Every other read happens under the family's PostgreSQL advisory lock, which belongs to the connection the write context opened, so those reads stay on the repository. Running them on a second connection would read outside the lock and silently break reuse detection, which is why `AcquireFamilyLockAsync` now reads the ambient transaction and throws when there is none, turning that requirement from a comment into a runtime invariant.
...
### Security
- **Auth reads can run on a lower-privilege connection (`ConnectionStrings:ReadOnlyConnection`)** - The read side of the refresh-token split resolves its own connection string, so `AuthReadOnlyDbContext` can be pointed at a PostgreSQL role holding nothing but `SELECT` on the `auth` schema. Defense in depth: an exploited read path then cannot write regardless of what the code attempts, and `AuthReadOnlyDbContext` also throws from `SaveChanges` so a write attempted through it fails at the call site during development rather than as a permission error in production. The key is **optional** and falls back to `ConnectionStrings:DefaultConnection` when unset, empty or whitespace — the empty-string case matters because Compose passes `${DB_READONLY_CONNECTION_STRING:-}` — so every existing environment keeps behaving exactly as it did. Compose plumbs the variable through to `movies.webservice` only; the role and its grants are a deployment step, documented in `.env.example`. Note the third registered `DbContext` never migrates: `AuthDbContext` still owns the schema, and passing `AuthReadOnlyDbContext` to `dotnet ef migrations add` would scaffold a duplicate migration set. Documented in the [Auth & Security Model](docs/auth-security-model.md).
