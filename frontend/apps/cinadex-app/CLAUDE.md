...
- **`main.tsx` imports `@cinedex/theme/tailwind.css` before `./index.css`.** That one import pulls in the tokens, the base element styling and Tailwind, in the cascade-layer order they have to be in — see [`packages/theme/CLAUDE.md`](../../packages/theme/CLAUDE.md). The design system belongs to the theme package; only app-specific layout lives in `src/index.css`.
