...
The templates: named layouts assembled from `@cinedex/frames`. The middle of the three component tiers.

Private and **source-consumed**, exactly like `@cinedex/frames` — `exports` point at `src/`, `npm run build` is a typecheck. Depends on `@cinedex/frames` and nothing else.
...
Same as [`@cinedex/frames`](../frames/CLAUDE.md): Tailwind only through `@cinedex/theme` tokens, `cn()` for class composition, named type steps rather than pixel values, one folder per component, barrel-only public surface, behaviour-and-a11y tests.

Import from `@cinedex/frames`, never by relative path into `../frames/src`.

## Notes

- `strengthFromRequirements` ships here rather than in frames — it is the pure helper `PasswordStrengthMeter` and `PasswordChecklist` are driven by, and has no meaning without them.
- `PasswordStrengthMeter` wraps frames' `ProgressBar`, which is Radix `Progress` — so the meter now has `role="progressbar"` and `aria-value*`, which the hand-rolled bar it replaced did not.
