...
export interface SceneLinkProps extends PropsWithChildren {
 ...
 * own `to`, so an already-built element cannot serve. `SceneProvider`
 * supplies a working `AnchorLink` default, which is what lets these screens
 * render router-free in Storybook and in tests, and `SceneProvider.test.tsx`
 ...
 */
export type SceneLinkComponent = ComponentType<SceneLinkProps>;
