...
- `aspire/Cinedex.AppHost` — Aspire orchestration for local dev (Postgres + Mailpit containers, the three hosts and the SPA's Vite dev server as processes). The SPA resource comes from `Aspire.Hosting.JavaScript`'s `AddViteApp` — Aspire 13 renamed that package from `Aspire.Hosting.NodeJs` and obsoleted `AddNpmApp`, which with warnings-as-errors means the old spelling will not compile. **No ServiceDefaults project on purpose**: `AddObservability` already consumes the `OTEL_EXPORTER_OTLP_*` variables the AppHost injects, and health endpoints already exist, so the only things ServiceDefaults would add are service discovery and HTTP resilience — neither of which this codebase uses. The `Aspire.AppHost.Sdk` version is pinned inline in the csproj because central package management does not cover MSBuild SDKs; keep it in step with the `Aspire.Hosting.*` versions in `Directory.Packages.props`.

The web service runs under its `aspire` launch profile (`Properties/launchSettings.json`), which is HTTP-only on port 9002. **Do not remove that profile or point the AppHost at `launchProfileName: null`.** Visual Studio launches Aspire project resources itself and reads a web project's URLs from its launch profile; without one, Kestrel binds 5000/5001 while Aspire's proxy forwards elsewhere, and the resource hangs at `Running (Unhealthy)`. `dotnet run` does not show this — there the orchestrator injects the port itself.
...
`Features:EnableFrontendUiSvc` (default `true`), same pattern, controls whether the SPA's Vite dev server runs. It and `EnableStorybookSvc` are the two resources that need Node and npm on `PATH`, and Aspire installs `frontend/apps/cinadex-app`'s dependencies for you when `node_modules` is missing (npm walks up to the `frontend/` workspace root, so that install is the whole workspace). On, the SPA is at `https://localhost:9000` (self-signed cert, so expect the browser warning) with `VITE_API_PROXY_TARGET` pointed at the AppHost's web service; the endpoint is pinned to port 9000 and unproxied, for the same reason the web service is — Vite binds with `strictPort` and that URL has to keep matching the web service's `Frontend:BaseUrl`, which password-reset links are built from. Off, the resource is omitted and the API is only reachable directly at `http://localhost:9002`.

`Features:EnableStorybookSvc` (default `true`), same pattern, controls whether the component library's Storybook runs. On, it serves at `http://localhost:9001` — plain http, since a workbench that calls no API has no TLS to terminate, and the same port `npm run storybook` uses so the two paths agree. It is the one resource that **references nothing and waits on nothing**: it renders `@cinedex/components` in isolation, so it stays useful with the whole backend switched off, and it is unaffected by `EnableFrontendUiSvc` in either direction. Shares that flag's Node-and-npm requirement. Note this is the dev server with hot reload; compose publishes the *built* static bundle on that same port — the two never run at once anyway, since Aspire and compose already collide on Postgres and the SPA. `AddViteApp` is pointed at the `storybook` script explicitly — this package has no `dev` script, which is that parameter's default.
