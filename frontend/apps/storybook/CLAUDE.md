...
- **`nginx.conf` serves the bundle under `/storybook/`, and the proxy in front of it must NOT strip that prefix.** `storybook build` emits every reference relative (`./sb-manager/runtime.js`, `./assets/*`, `./brand.svg`, and `manager.ts`'s `./brand.svg`), so the bundle runs under any prefix — verified against `storybook-static/`, which contains no absolute-root asset path at all. The catch is the trailing slash: at `/storybook` without one, the browser resolves `./sb-manager/runtime.js` against the _parent_ and requests `/sb-manager/runtime.js` at the site root, where the SPA's `try_files ... /index.html` catch-all returns HTML at **200**. Blank Storybook, no console error worth the name, green build, green deploy. The `location = /storybook { return 308 /storybook/; }` redirect is the only fix, and nginx can only issue it if it can see the prefix — which is why stripping it upstream (Dokploy's **Strip Path**, Caddy's `uri strip_prefix`) breaks this in a way nothing in the build catches. `location = / { return 308 /storybook/; }` is what keeps the bare `http://localhost:9001` in compose working. Same shape as [`@cinedex/docs-site`](../docs-site/CLAUDE.md)'s conf; see [`docs/deployment-dokploy.md`](../../../docs/deployment-dokploy.md).
