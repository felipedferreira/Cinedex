...
### Added
- **The SPA runs under the Aspire AppHost (`AddCinedexFrontendUi`)** - `dotnet run --project aspire/Cinedex.AppHost` now starts `frontend/apps/cinadex-app`'s Vite dev server alongside Postgres, Mailpit and the three .NET hosts, so the inner loop covers the whole stack rather than the backend half of it. Previously the SPA was only part of the Compose path, and using it with the AppHost meant a second terminal running `npm run dev` and remembering that its `/movies-svc` proxy defaults to the bare `dotnet run` address, not the one the AppHost gives the web service.
...
- **Storybook as its own app (`@cinedex/storybook`)** - Storybook 10 with `@storybook/react-vite`, `addon-docs` for generated docs and `addon-a11y` for accessibility checks, at `frontend/apps/storybook`. `npm run storybook` from `frontend/` serves it on port 6006 — the SPA holds 9000 with `strictPort: true` — and Compose serves the built bundle on `http://localhost:9001`. CI runs `build-storybook`, so a broken story or a bad config fails the build rather than being discovered later.
...
### Changed
- **`frontend/` is now an npm workspace (`apps/*` + `packages/*`)** - The SPA moved from `frontend/cinadex-ui/` to `frontend/apps/cinadex-app/` and shares the tree with the Storybook app at `frontend/apps/storybook/` and the library at `frontend/packages/components/`. The single `package-lock.json` and all shared tooling config — `eslint.config.js`, `.prettierrc.json`, `.prettierignore`, `.gitignore`, `.dockerignore` — are hoisted to `frontend/`, which is where every npm command now runs. Lint and format make one pass over every package; build, test and coverage fan out with `--workspaces`; `-w cinadex-app`, `-w @cinedex/storybook` or `-w @cinedex/components` scopes any of them to one. Both apps depend on the library; nothing depends on an app.
  ...
  - **The Aspire AppHost's SPA resource follows the move.** `AppHostConstants.FrontendAppDirectory` now points at `frontend/apps/cinadex-app`. It deliberately stays on the package rather than the workspace root: `npm run dev` there resolves to plain `vite`, so the `--port` Aspire appends reaches it, which it would not through the root script's `-w cinadex-app` delegation.
