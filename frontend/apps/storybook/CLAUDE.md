...
- `staticDirs` points at `../../cinedex-app/public` so the `/icons.svg#id` sprite resolves in stories, and at `./public` (this app's own) for `manager.ts`'s brand asset — both are copied into `storybook-static/` on `build-storybook`.
