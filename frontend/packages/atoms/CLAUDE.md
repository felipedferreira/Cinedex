...
The primitives: Radix-backed, Tailwind-styled, one job each. The bottom of the three component tiers — `@cinedex/compounds` assembles these, `@cinedex/solution` assembles those.
...
- **`Button` covers four looks and three sizes in one component** — `primary`/`ghost` (the landing page's tinted style) and `solid`/`outline` (the auth flow's ink-fill CTA), at `sm`/`md`/`block`. `size="block"` sets `display: block`, which `cn()` resolves against the base `inline-flex`.
...
- A new component is not visible to Storybook until it is exported from `src/index.ts` — the stories import from `@cinedex/atoms`, not by relative path.
