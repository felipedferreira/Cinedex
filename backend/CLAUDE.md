...
`Features:EnableFrontendUiSvc` (default `true`), same pattern, controls whether the SPA's Vite dev server runs. It is the only resource that needs Node and npm on `PATH`, and Aspire installs `frontend/apps/cinadex-app`'s dependencies for you when `node_modules` is missing (npm walks up to the `frontend/` workspace root, so that install is the whole workspace). On, the SPA is at `https://localhost:9000` (self-signed cert, so expect the browser warning) with `VITE_API_PROXY_TARGET` pointed at the AppHost's web service; the endpoint is pinned to port 9000 and unproxied, for the same reason the web service is — Vite binds with `strictPort` and that URL has to keep matching the web service's `Frontend:BaseUrl`, which password-reset links are built from. Off, the resource is omitted and the API is only reachable directly at `http://localhost:5187`.
